...
namespace WiFi_Functions
{
    ...
    AP_Credentials *latestAPConnection = nullptr;
    esp_event_handler_instance_t instance_any_id;
    esp_event_handler_instance_t instance_got_ip;

    EventGroupHandle_t s_wifi_event_group;
    esp_netif_t *sta_netif;
    std::vector<AP_Credentials *> AP_Credentials::vec_AP_Credentials;

    AP_Credentials *AP_Credentials::saveRouterConfiguration(std::string wifi_ssid, std::string wifi_password, bool save_to_nvs)
    {
        for (auto connection : vec_AP_Credentials)
        {
            if (connection->getWifi_ssid() != "" && connection->getWifi_ssid() == wifi_ssid)
            {
                ...
                AP_Credentials::setCredentialToNVS(connection->getWifi_ssid(), connection);
        ...
        AP_Credentials *newConnection = new AP_Credentials(wifi_ssid, wifi_password);
        vec_AP_Credentials.push_back(newConnection);
        AP_Credentials::setCredentialToNVS(wifi_ssid, newConnection);
    ...
    AP_Credentials *AP_Credentials::getCredentialFromNVS(std::string wifi_ssid)
    {
        ...
        AP_Credentials *returnptr = new AP_Credentials(ssid, password);
        return returnptr;
    }

    esp_err_t AP_Credentials::setCredentialToNVS(std::string wifi_ssid, AP_Credentials *cred)
    {
    ...
    bool AP_Credentials::loadAllCredentialsFromNVS(std::string p_nvs_namespace_name)
    {
        ...
        do
        {
            ...
            if (err == ESP_OK)
            {
                ...
                if (received_string.size() > 0)
                {
                    ...
                    AP_Credentials *newConnection = new AP_Credentials(ssid, password);
                    vec_AP_Credentials.push_back(newConnection);
        ...
        nvs_close(nvs_wrapper::nvs_namespace_handle);
        return true;
    }
     ...
     * @throws None.
     */
    AP_Credentials *AP_Credentials::findSavedRouterConnection(std::string wifi_ssid)
    {
    ...
    {
        constexpr char *TAG = "wifi_event_handler:";
        ...
        while (1)
        {
            ...
            while (1)
            {
                ...
                while (tries < 10)
                {
                    ...
                    std::cout << "\n\n\ntry number:<< " << tries << "\n\n\n";
                    std::cout << "1\n";
                    availability = check_network_availability(latestAPConnection->getWifi_ssid());
                    ...
                    if (availability == 2)
                    {
                        ...
                        std::cout << "Unexpected Error while testing for availability of " << latestAPConnection->getWifi_ssid() << "\nNext try in 5 secs.\n";
                    ...
                    else if (availability == 0)
                    {
                        ...
                        std::cout << latestAPConnection->getWifi_ssid() << " is currently not available.\nNext try in 30 secs.\n";
                    ...
                    else
                    {
                        ...
                        {
                            std::cout << latestAPConnection->getWifi_ssid() << " did not accept connection request.\nNext try in 30 secs.\n";
                            vTaskDelay(30000 / portTICK_PERIOD_MS);
                        }
                        else
                        {
                ...
                std::vector<std::string> available_networks = find_available_networks();
                for (auto it : available_networks)
                {
                    if (connect_to_ap(AP_Credentials::findSavedRouterConnection(it)))
                    {
                        std::cout << "Alternative Connection to " << AP_Credentials::findSavedRouterConnection(it)->getWifi_ssid() << " was successfull.\n";
                        return;
                    }
                }
            }
    ...
    bool connect_to_ap(AP_Credentials *connection)
    {
        ...
        EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdTRUE, pdFALSE, 120000 / portTICK_PERIOD_MS);
        ...
        if (bits & WIFI_CONNECTED_BIT)
        {
            ...
            latestAPConnection = connection;

            xTaskCreate(WiFi_Task::Task_Start_Fail_Bit_Listener, "Task_Start_Fail_Bit_Listener", 8 * 1024, NULL, 6, NULL);
            return true;
        }
        else if (bits & WIFI_FAIL_BIT)
        {
            printf("\nFailed to connect to AP using the credentials:\n\t%s\n\t%s\n", connection->getWifi_ssid().c_str(), connection->getWifi_password().c_str());
        }
        else
        {
            printf("\nUNEXPECTED ERROR while establishing initial AP Connection.");
        }
    ...
    std::vector<std::string> find_available_networks()
    {
        ...
        {
            for (auto it : AP_Credentials::getVecAPCredentials())
            {
    ...
    bool connect_to_nearest_ap()
    {
        ...
        return connect_to_ap(AP_Credentials::findSavedRouterConnection(get<0>(best_ap)));
    }

    void AP_Credentials::listAllConnections()
    {
        int i = 0;
        for (auto it : vec_AP_Credentials)
        {
    ...
    void AP_Credentials::printConnectionData()
    {
...
void WiFi_Task::Task_Start_Fail_Bit_Listener(void *params)
{
